...
RUN wget -nv -O /builds/worker/fetches/clang.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/linux64-clang-13-win-cross/clang.tar.zst && \
    cd /builds/worker/fetches/ && \
    tar -xf clang.tar.zst && \
    rm clang.tar.zst

RUN wget -nv -O /builds/worker/fetches/rustc.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/linux64-rust-cross-1.57/rustc.tar.zst && \
    cd /builds/worker/fetches/ && \
...
RUN wget -nv -O /builds/worker/fetches/wine.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/linux64-wine/wine.tar.zst && \
    cd /builds/worker/fetches/ && \
    tar -xf wine.tar.zst && \
    rm wine.tar.zst

RUN wget -nv -O /builds/worker/fetches/liblowercase.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/linux64-liblowercase/liblowercase.tar.zst && \
    cd /builds/worker/fetches/ && \
    tar -xf liblowercase.tar.zst && \
    rm liblowercase.tar.zst

RUN wget -nv -O /builds/worker/fetches/winchecksec.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/linux64-winchecksec/winchecksec.tar.zst && \
    cd /builds/worker/fetches/ && \
    tar -xf winchecksec.tar.zst && \
    rm winchecksec.tar.zst

RUN wget -nv -O /builds/worker/fetches/nsis.tar.zst https://ghostery-user-agent-cache-public.s3.amazonaws.com/toolchains/96.0.1/nsis/nsis.tar.zst && \
    cd /builds/worker/fetches/ && \
    tar -xf nsis.tar.zst && \
    rm nsis.tar.zst
