...
do
  ...
  mkdir -p $(dirname $DMG)/update
  mozilla-release/tools/update-packaging/make_full_update.sh $(dirname $DMG)/update/${DMG_NAME%.dmg}.complete.mar "$PKG_DIR/$PKG_NAME.app"
done
